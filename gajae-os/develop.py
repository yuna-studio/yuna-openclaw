...
def _make_review_prompt(state: DevState) -> str:
    ...
    review_focus = {
        ...
        6: ("목적 부합 검토",
            "구현된 코드가 설계 문서의 목적에 맞는가? 기능이 정확히 동작하는가?",
            ["기능 완전성", "설계 일치", "동작 정확성"]),
        7: ("버그/크리티컬/보안",
            "잠재적 버그, 크리티컬 이슈, 보안 취약점이 있는가?",
            ["잠재적 버그", "크리티컬 이슈", "보안 취약점", "에러 핸들링"]),
        8: ("개선 내용 검토",
        ...
        14: ("코드 품질",
            ...
            ["가독성", "유지보수성", "타입 안전성", "에러 처리"]),
        15: ("UX 흐름",
            "사용자의 실제 사용 흐름에서 문제가 없는가? 엣지 케이스는?",
            ["사용자 흐름", "엣지 케이스", "로딩/에러 상태", "모바일 UX"]),
        16: ("연쇄 영향 반복 검토",
        ...
        17: ("배포 퀄리티",
            ...
            ["배포 준비", "환경 설정", "빌드 성공", "성능"]),
    }

    title, question, criteria = review_focus.get(step, ("검토", "문제가 없는가?", ["품질"]))
    criteria_text = "\n".join(f"{i+1}. **{c}**" for i, c in enumerate(criteria))

    return f"""너는 Staff Engineer급 시니어 코드 리뷰어다. 실무 10년+.
대충 넘어가지 마라. 실제 프로덕션 코드를 리뷰하듯 꼼꼼하게.
...
ISSUES: (발견된 구체적 문제 목록)
FIXES: (각 문제에 대한 구체적 수정 지시)
